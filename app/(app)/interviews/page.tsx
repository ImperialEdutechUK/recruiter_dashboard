...
export default async function InterviewsPage() {
  ...
  const { data: rows } = await supabase.from("candidates").select("*").in("current_stage", [3, 4]).order("interview_date");
  const tableRows = await enrichRows(supabase, (rows ?? []) as Candidate[]);

  return (
    <div className="space-y-6">
      <PageHeader title="Interviews" subtitle="Scheduled and recently conducted interviews (Stages 3–4)" />
